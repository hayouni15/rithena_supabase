...
export function runQualityChecks(job: QualityJob, brief: CreativeBrief, media: {
  ...
  for (const platform of platforms) {
    ...
    if (hashtags.length < 1) copyIssues.push(issue(
      "platform_hashtag_required", `${platform} requires at least one hashtag.`, `Provide one relevant ${platform} hashtag.`,
      { field: "hashtags", platform, quote: hashtags.join(" "), suggestion: `Add one relevant ${platform} hashtag.` },
    ));
  }
