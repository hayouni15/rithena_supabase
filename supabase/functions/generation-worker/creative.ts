...
export function creativeBriefPrompt(job: BriefJob) {
  ...
  const imageStrategy = input.imageStrategy && typeof input.imageStrategy === "object" ? input.imageStrategy as Record<string, unknown> : {};
  const textStrategy = String(imageStrategy.textStrategy || "hybrid");
...
${job.type === "image" && !carousel ? `SELECTED IMAGE RECIPE:
${JSON.stringify(imageStrategy, null, 2)}
` : ""}
...
${textStrategy === "structured_overlay" ? `- Generate imagery only and deliberately reserve a low-detail area for Rithena's deterministic text composition.
...
- Keep the principal subject clear of the reserved overlay area and mobile-safe margins.` : `- Treat imagery, expressive typography, spacing, and hierarchy as one coherent art-direction decision.
- The media_prompt must include the exact headline and supporting copy from text_overlay and explain how that typography participates in the visual idea.
- Do not ask the model to render the brand logo, CTA, URL, price, statistic, disclaimer, or any other precision-sensitive field. Reserve safe space for Rithena to add exact overlays.`}
`}
TEXT OVERLAY
