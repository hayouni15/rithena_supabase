...
type Credential = { ciphertext:string; organizationId:string; brandId:string; connectionId:string; accountId:string; platform:"instagram"|"facebook"|"linkedin"|"youtube"|"tiktok" };
...
async function resources(db:Db,job:Job){
  ...
  let query=db.from("media_assets").select("id,asset_type,storage_bucket,storage_path,mime_type,file_size_bytes,duration_seconds,metadata").eq("organization_id",job.organization_id).eq("content_item_id",job.content_item_id).eq("status","ready");
...
async function tiktokToken(decrypted:string){
  let stored:{accessToken?:string;refreshToken?:string;expiresAt?:string;refreshExpiresAt?:string};try{stored=JSON.parse(decrypted);}catch{throw new PublishError("credential_invalid","TikTok credentials could not be read. Reconnect TikTok.",false);}
  if(stored.accessToken&&stored.expiresAt&&Date.parse(stored.expiresAt)>Date.now()+60_000)return stored.accessToken;if(!stored.refreshToken||!stored.refreshExpiresAt||Date.parse(stored.refreshExpiresAt)<=Date.now())throw new PublishError("tiktok_expired","TikTok access expired. Reconnect the account, then reschedule.",false);
  let response:Response;try{response=await fetch("https://open.tiktokapis.com/v2/oauth/token/",{method:"POST",headers:{"content-type":"application/x-www-form-urlencoded","cache-control":"no-cache"},body:new URLSearchParams({client_key:env("TIKTOK_CLIENT_KEY"),client_secret:env("TIKTOK_CLIENT_SECRET"),grant_type:"refresh_token",refresh_token:stored.refreshToken}),signal:AbortSignal.timeout(25_000)});}catch{throw new PublishError("tiktok_unavailable","TikTok authentication is temporarily unavailable. Rithena will retry automatically.",true);}let body:Json={};try{body=await response.json() as Json;}catch{/* classified below */}if(!response.ok||typeof body.access_token!=="string"){if(response.status===401||body.error==="invalid_grant")throw new PublishError("tiktok_revoked","TikTok access was removed. Reconnect the account, then reschedule.",false);throw new PublishError("tiktok_unavailable","TikTok authentication is temporarily unavailable. Rithena will retry automatically.",response.status>=500||response.status===429);}return body.access_token;
}
async function tiktok(path:string,token:string,init:RequestInit={}):Promise<Json>{
  let response:Response;try{response=await fetch(`https://open.tiktokapis.com/v2/${path}`,{...init,headers:{authorization:`Bearer ${token}`,"content-type":"application/json; charset=UTF-8",...init.headers},signal:AbortSignal.timeout(45_000)});}catch{throw new PublishError("tiktok_unavailable","TikTok could not be reached. Rithena will retry automatically.",true);}let body:Json={};try{body=await response.json() as Json;}catch{/* classified below */}const apiError=body.error&&typeof body.error==="object"?body.error as Json:{};const code=String(apiError.code||"");if(!response.ok||(code&&code!=="ok")){const message=String(apiError.message||"TikTok rejected this post.").slice(0,400);if(response.status===401||code==="access_token_invalid")throw new PublishError("tiktok_revoked","TikTok access was removed. Reconnect the account, then reschedule.",false);if(response.status===403||code==="scope_not_authorized")throw new PublishError("tiktok_permissions",message,false);if(response.status===429||code==="rate_limit_exceeded"||response.status>=500)throw new PublishError("tiktok_unavailable","TikTok is temporarily unavailable. Rithena will retry automatically.",true);throw new PublishError(`tiktok_${code||response.status}`,message,false);}return body;
}
async function runTikTok(db:Db,job:Job,worker:string,token:string){
  const source=await resources(db,job);const asset=source.assets[0];if(!asset||!["video/mp4","video/quicktime","video/webm"].includes(String(asset.mime_type||"")))throw new PublishError("tiktok_video_required","TikTok publishing requires a finished MP4, MOV, or WebM video.",false);const payload=job.provider_payload||{};const settings=payload.settings&&typeof payload.settings==="object"?payload.settings as Json:{};
  if(job.provider_job_id){const body=await tiktok("post/publish/status/fetch/",token,{method:"POST",body:JSON.stringify({publish_id:job.provider_job_id})});const data=body.data&&typeof body.data==="object"?body.data as Json:{};const status=String(data.status||"").toUpperCase();if(status==="FAILED"){const reason=Array.isArray(data.fail_reason)?data.fail_reason.join("; "):String(data.fail_reason||"TikTok could not publish this video.");throw new PublishError("tiktok_processing_failed",reason.slice(0,400),false);}if(status!=="PUBLISH_COMPLETE"){const polls=Number(payload.processingPolls||0)+1;if(polls>80)throw new PublishError("tiktok_processing_timeout","TikTok did not finish publishing this video within the expected time.",false);await checkpoint(db,job,worker,"waiting_external",{providerId:job.provider_job_id,payload:{...payload,stage:"processing",processingPolls:polls},retryAfter:15});return;}const publicIds=Array.isArray(data.publicaly_available_post_id)?data.publicaly_available_post_id:[];const remoteId=typeof publicIds[0]==="string"?publicIds[0]:job.provider_job_id;await checkpoint(db,job,worker,"succeeded",{providerId:job.provider_job_id,payload:{...payload,stage:"published",remotePostId:remoteId},remoteId,remoteUrl:remoteId===job.provider_job_id?undefined:`https://www.tiktok.com/@user/video/${remoteId}`});return;}
  const bytes=await assetBytes(db,asset);const chunkSize=bytes.byteLength<5_000_000?bytes.byteLength:Math.min(10_000_000,bytes.byteLength);const chunkCount=Math.max(1,Math.floor(bytes.byteLength/chunkSize));let uploadUrl=typeof payload.uploadUrl==="string"?payload.uploadUrl:"";let publishId=typeof payload.publishId==="string"?payload.publishId:"";
  if(!uploadUrl||!publishId){const creator=await tiktok("post/publish/creator_info/query/",token,{method:"POST",body:"{}"});const info=creator.data&&typeof creator.data==="object"?creator.data as Json:{};const privacy=String(settings.privacyLevel||"SELF_ONLY");const options=Array.isArray(info.privacy_level_options)?info.privacy_level_options.map(String):[];if(!options.includes(privacy))throw new PublishError("tiktok_privacy_unavailable","The selected TikTok privacy setting is no longer available. Review the TikTok publishing options and schedule again.",false);const duration=Number(source.assets[0]?.duration_seconds||0);const maxDuration=Number(info.max_video_post_duration_sec||0);if(duration>0&&maxDuration>0&&duration>maxDuration)throw new PublishError("tiktok_video_too_long",`This TikTok account accepts videos up to ${maxDuration} seconds.`,false);const title=source.caption.slice(0,2200);const initialized=await tiktok("post/publish/video/init/",token,{method:"POST",body:JSON.stringify({post_info:{title,privacy_level:privacy,disable_comment:Boolean(settings.disableComment)||info.comment_disabled===true,disable_duet:Boolean(settings.disableDuet)||info.duet_disabled===true,disable_stitch:Boolean(settings.disableStitch)||info.stitch_disabled===true,brand_content_toggle:false,brand_organic_toggle:true,is_aigc:true},source_info:{source:"FILE_UPLOAD",video_size:bytes.byteLength,chunk_size:chunkSize,total_chunk_count:chunkCount}})});const data=initialized.data&&typeof initialized.data==="object"?initialized.data as Json:{};uploadUrl=String(data.upload_url||"");publishId=String(data.publish_id||"");if(!uploadUrl||!publishId)throw new PublishError("tiktok_invalid_response","TikTok did not initialize the video upload.",true);await checkpoint(db,job,worker,"waiting_external",{payload:{...payload,stage:"uploading",uploadUrl,publishId,nextChunk:0,chunkSize,chunkCount},retryAfter:1});return;}
  const nextChunk=Number(payload.nextChunk||0);const storedCount=Number(payload.chunkCount||chunkCount);const storedSize=Number(payload.chunkSize||chunkSize);const first=nextChunk*storedSize;const last=nextChunk===storedCount-1?bytes.byteLength-1:Math.min(bytes.byteLength-1,first+storedSize-1);if(first>last)throw new PublishError("tiktok_invalid_response","TikTok returned an invalid upload position.",false);let response:Response;try{response=await fetch(uploadUrl,{method:"PUT",headers:{"content-type":asset.mime_type,"content-length":String(last-first+1),"content-range":`bytes ${first}-${last}/${bytes.byteLength}`},body:bytes.slice(first,last+1),signal:AbortSignal.timeout(120_000)});}catch{throw new PublishError("tiktok_unavailable","TikTok could not receive the video chunk. Rithena will retry automatically.",true);}if(!response.ok)throw new PublishError(response.status>=500?"tiktok_unavailable":"tiktok_upload_failed","TikTok could not receive the video upload.",response.status>=500);if(nextChunk+1<storedCount){await checkpoint(db,job,worker,"waiting_external",{payload:{...payload,stage:"uploading",uploadUrl,publishId,nextChunk:nextChunk+1,chunkSize:storedSize,chunkCount:storedCount},retryAfter:1});return;}await checkpoint(db,job,worker,"waiting_external",{providerId:publishId,payload:{settings,stage:"processing",processingPolls:0},retryAfter:10});
}
...
Deno.serve(async(request)=>{
  ...
  try {
    ...
    else if(credential.platform==="tiktok")await runTikTok(db,job,worker,await tiktokToken(decrypted));
