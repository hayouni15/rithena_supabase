...
function template(delivery:Delivery,firstName:string){
  const p=delivery.payload||{};const action=text(p,"actionUrl","/home");const handle=text(p,"handle","Instagram").replace(/^@/,"");const platform=text(p,"platform","Social");
  const messages:Record<string,{subject:string;preheader:string;eyebrow:string;heading:string;body:string;cta:string}>={
    ...
    content_published:{subject:`Published: ${text(p,"title","Your content")}`,preheader:`Your content is now live on ${text(p,"platform","social media")}.`,eyebrow:`LIVE ON ${text(p,"platform","SOCIAL MEDIA").toUpperCase()}`,heading:"Your content is published.",body:`${text(p,"title","Your post")} is now live on ${text(p,"platform","social media")}.`,cta:"Open published post"},
    ...
    publishing_failed:{subject:`${platform} publishing needs attention`,preheader:"A scheduled post did not publish.",eyebrow:"ACTION NEEDED",heading:`A ${platform} post did not publish.`,body:text(p,"message","Open the post to review the failure and reschedule it."),cta:"Open the post"},
