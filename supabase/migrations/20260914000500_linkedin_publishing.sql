-- LinkedIn Company Page scheduling through the durable social publish queue.

create or replace function public.schedule_linkedin_content(p_content_item_id uuid,p_expected_revision integer,p_scheduled_for timestamptz)
...
begin
 ...
 if item.status not in ('approved','failed') or not ('linkedin'=any(item.platform_targets)) then raise exception 'Approve the LinkedIn version before scheduling' using errcode='22023'; end if;
 ...
 select * into variant from public.platform_variants where content_item_id=item.id and organization_id=item.organization_id and platform='linkedin' and status='ready' for update;
 if variant.id is null or variant.selected_media_asset_id is null or not exists(select 1 from public.media_assets m where m.id=variant.selected_media_asset_id and m.status='ready') or not exists(select 1 from public.post_copies p where p.platform_variant_id=variant.id and p.is_selected) then raise exception 'The finished LinkedIn media and copy are not ready' using errcode='22023'; end if;
 select * into connection from public.social_connections where brand_id=item.brand_id and platform='linkedin' and status='connected' and token_expires_at>now() and 'w_organization_social'=any(scopes) order by last_validated_at desc nulls last limit 1 for update;
 if connection.id is null then raise exception 'Connect or reconnect LinkedIn before scheduling' using errcode='P0001'; end if;
 ...
 if item.status='failed' then
  select j.* into job from public.publish_jobs j join public.platform_variants v on v.id=j.platform_variant_id where j.content_item_id=item.id and j.content_revision=item.content_revision and j.state='failed' and v.platform='linkedin' order by j.created_at desc limit 1 for update of j;
  if job.id is null then raise exception 'This failed post cannot be rescheduled' using errcode='22023'; end if;
  if job.error_code='publish_outcome_unknown' then raise exception 'Check LinkedIn first because the previous publish result is unknown' using errcode='22023'; end if;
 ...
 values(item.organization_id,result.id,item.id,variant.id,connection.id,item.content_revision,'queued','linkedin:'||item.id::text||':revision:'||item.content_revision::text,p_scheduled_for,5);
 perform set_config('rithena.lifecycle_transition','allowed',true);
 update public.content_items set status='scheduled',failure_code=null,failure_message=null where id=item.id;
 return result;
end $$;
...
create or replace function public.cancel_linkedin_schedule(p_content_item_id uuid,p_expected_revision integer) returns public.schedules language plpgsql security definer set search_path='' as $$
declare job public.publish_jobs; schedule public.schedules; item public.content_items;
begin
 select j.* into job from public.publish_jobs j join public.platform_variants v on v.id=j.platform_variant_id where j.content_item_id=p_content_item_id and v.platform='linkedin' and public.is_organization_member(j.organization_id) order by j.created_at desc limit 1 for update of j;
...
revoke all on function public.schedule_linkedin_content(uuid,integer,timestamptz),public.cancel_linkedin_schedule(uuid,integer) from public,anon;
grant execute on function public.schedule_linkedin_content(uuid,integer,timestamptz),public.cancel_linkedin_schedule(uuid,integer) to authenticated;
