...
const productWorkerUrl = `https://${projectRef}.supabase.co/functions/v1/product-generation-worker`;
...
begin
  ...
  select id into existing_id from vault.secrets where name = 'rithena_product_worker_url' order by created_at desc limit 1;
  if existing_id is null then
    perform vault.create_secret(${literal(productWorkerUrl)}, 'rithena_product_worker_url', 'Rithena product-reference Edge worker URL');
  else
    perform vault.update_secret(existing_id, ${literal(productWorkerUrl)}, 'rithena_product_worker_url', 'Rithena product-reference Edge worker URL');
  end if;
end
$configure$;
`;
...
console.log(`Generation crons configured for ${workerUrl} and ${productWorkerUrl}.`);
